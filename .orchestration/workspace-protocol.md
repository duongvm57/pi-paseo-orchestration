...
current-run Task Authority Envelope > ordinary task prose.
...
- **Supervisor:** observes and proposes. It may inspect evidence, request
  ...
  Recovery remains grant-bound. The Supervisor may write a protocol revision
  only after exact direct Human confirmation of the complete diff; the
  protocol itself grants no capability.
- **Lead:** is the technical arbiter for reversible technical decisions within
  ...
  every Human `must_ask` decision and cannot claim Local Acceptance.
...
- **Tiny/bounded:** A local, bounded, reversible change that does not cross
  ...
  specific tiny case and a separate exact Human authority envelope supplies
  the capability.
...
- The Lead assigns one accountable owner for each exact moving scope and path
  ...
  an authority change also requires a new direct Human grant.
...
- **`REOPEN_REQUEST`:** A Peer may issue it when a foundational premise, API or
  ...
  requires the appropriate new assignment or grant.
...
- **Evidence required:** The active Role Profile, pinned protocol, current-run
  Task Authority Envelope, effective tool set, and requested action.
...
- **Allowed response:** Stop the action unless the complete authority chain is
  present; request the missing Human grant without widening scope.
