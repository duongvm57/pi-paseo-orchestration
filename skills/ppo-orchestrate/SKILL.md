...
compatibility: Requires the paseo MCP server and configured PPO role aliases. A repository Workspace Protocol is needed only for protocol-derived routing, Lead self-write, or candidate acceptance.
---
...
1. Verify before protocol-derived orchestration: your own Paseo identity is observable, `ParentAgentId` is absent, provider/role is Lead, and required Paseo MCP operations are connected and discoverable. If repository-specific rules or Lead self-write are needed, pin a valid Workspace Protocol; otherwise continue in core mode.
2. Accept the Human task directly. The initial Human task and each exact Peer assignment authorize ordinary inspection and testing; a worktree remains an isolation option. Lead edit/write/local-commit requires protocol opt-in, and Peer edit/write/local-commit requires the Engineer `write_mode` binding. Do not ask the Human to write markers, JSON, hashes, agent IDs, assignment IDs, scope syntax, capability lists, digests, or grants.
3. When a Workspace Protocol exists and applies, read and pin it before using its rules. Use `paseo_list_workspaces` and `paseo_list_providers` only when discovery is needed. Run `/ppo:doctor` for setup, recovery, suspected configuration/runtime drift, or troubleshooting—not as a gate for normal core-mode delegation and never ask the Human to copy its output between agents.
...
6. Treat a worktree as an isolation option, not a default. Propose one when concurrent writers or review risk justify it, and let the Human choose or confirm the checkout trade-off; otherwise serialize the moving scope. A dirty caller checkout is evidence to classify, not an automatic blocker: untracked/read-only issue notes, specifications, research, generated logs, or unrelated documentation do not block a chosen isolated worktree from a known clean commit. Only a real collision, overwrite risk, competing writer, or ambiguous base blocks work.
...
10. For candidate-producing write work, require the exact Stable Candidate and every protocol-mandated independent review before issuing `READY`, `NOT_READY`, or `NEEDS_HUMAN`. Stop at direct Human Local Acceptance. Core-mode read-only work does not require a Git candidate.
...
A Peer uses `peer_lead_message` for mid-run `question`, `blocked`, `dependency`, or `progress`, resolving the recipient from the process `ParentAgentId`. On `agent_end`, the runtime validates the terminal Peer Report against the active Peer/parent identities and automatically delivers it to that parent as `handoff`, `blocked`, `dependency`, `question` (for reopen), or `progress`. A Supervisor may ask a bound Lead or relay a Human decision to that Lead; it never messages Peers or issues project acceptance. A Supervisor proposes a successor Lead only with evidence and a bounded handoff; the Human creates the root successor Lead, because agent-scoped `create_agent` cannot mint a root agent. The Supervisor relays the bounded handoff to that successor.

## Peer assignment contract

A read-only Peer ends with the exact closed report as the first nonempty final content. Preserve every field name and JSON shape. The runtime delivers a valid correlated terminal report to the exact parent Lead; delivery failure is surfaced in the Peer process while Paseo's finish notification remains available for recovery. The Lead copies the terminal template for the disposition (HANDOFF for read-only, the Workspace Protocol's Stable Candidate contract for write-producing assignments) into the Peer brief and binds identity placeholders before launch.
