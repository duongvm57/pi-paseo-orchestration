...
61. As a Human, I want exact Supervisor and Lead model selections plus a closed allowlist of named Peer model routes, so that Lead can fit each assignment without hidden defaults or unapproved models.
...
- `/ppo:settings` is always registered, invokes no model, and is the only package surface that changes PPO model settings or installs the three PPO Paseo provider profiles. Its Role models flow uses Pi's current model registry to collect exact Supervisor and Lead selections and one selection for every Peer route, shows the complete replacement, and writes only after Human confirmation. Its Paseo profiles flow replaces only `agents.providers.ppo-supervisor`, `ppo-lead`, and `ppo-peer`, then reports that Paseo must restart.
- The complete closed document lives at `<effective Pi config directory>/pi-paseo-orchestration/settings.json`. Version 2 contains exact `supervisor` and `lead` entries plus `peer_routes`. Routes `fast`, `general`, `reasoning`, `coding`, and `architecture` remain required for v2 compatibility; the wizard also writes the built-in `reviewer` route, and Human-named custom routes may be added. Every route has a bounded description and exact `provider`, `model`, and `thinking`; `thinking` is passed unchanged as Paseo `settings.thinkingOptionId`. A valid v1 document is read as v2 by expanding its former Peer selection across the five required routes, without mutating the file until Human confirmation.
- The document is Human-owned runtime state, not package data, project policy, Paseo lifecycle state, disposition, or authority. Cancel, incomplete selection, unknown model, invalid route, or invalid thinking preserves prior bytes. A valid replacement affects fresh governed processes only; it does not hot-switch an active process.
- Fresh Supervisor and Lead processes apply their fixed selection. A fresh Peer must already have an effective model/thinking tuple matching at least one snapshotted route; activation rejects an unapproved tuple rather than silently replacing it. Later file or runtime drift blocks ordinary prompts while settings and doctor remain available.
- Lead selects one configured route per assignment, binds its exact route ID in the Peer brief, and creates the child with that route's exact provider/model/thinking. Route selection is independent from the Lead-authored open-vocabulary disposition and grants no authority. Supervisor uses only the fixed Lead selection for authorized recovery.
- There is no automatic model router, per-project override, resolver service, remote-host route, compatibility matrix, fallback, or package-selected model default.
...
- One Peer Role Profile supports open-vocabulary assignment dispositions; Engineer, Architect, Reviewer, and Scout are common examples rather than an enum. Disposition narrows the mandate but never changes identity or authority.
...
- Lead mints assignment identity before Paseo child creation, selects one Human-configured Peer model route for the assignment, binds the route ID and returned exact child ID, and launches with that route's exact tuple. Lead may vary routes by task but cannot use a model outside the snapshotted allowlist. Failure or ambiguity permits at most one bounded inspection; a deliberate retry uses a new assignment identity.
