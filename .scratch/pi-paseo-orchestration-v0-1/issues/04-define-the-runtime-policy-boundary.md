...
This follows the normative reference orchestration model separation between durable Role Profile, repository Workspace Protocol, current-run task authority, and Paseo lifecycle truth. The two resolved research tickets establish the supported Pi hooks and the minimal fail-closed mechanism. `Minnyat/paseo-pi-team` remains implementation evidence only, not a dependency or compatibility target; v0.1 intentionally differs by intersecting existing active tools, rejecting an explicitly invalid role, avoiding regex authorization of `mcp_script`, and keeping authority state session-local.
